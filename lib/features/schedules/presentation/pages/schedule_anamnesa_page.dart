...
class ScheduleAnamnesaPage extends StatefulWidget {
  ...
  existingAnamnesaData; // Optional: to avoid API call if already have data

  const ScheduleAnamnesaPage({
    super.key,
    required this.registrationId,
    this.onCompleted,
    this.existingAnamnesaData,
  });
...
class _ScheduleAnamnesaPageState extends State<ScheduleAnamnesaPage> {
  ...
  Future<void> _fetchExistingAnamnesa() async {
    ...
    try {
      ...
      if (widget.existingAnamnesaData != null) {
        debugPrint('✅ Using existing anamnesa data from widget parameter');
        // Data from navigation parameter is already the anamnesa object itself
        data = widget.existingAnamnesaData!;
      } else {
        debugPrint('🌐 Fetching existing anamnesa from API');
        final regRepo = getIt<RegistrasiRepository>();
        // Assume Registrasi raw includes flat `anamnesa` object with fields
        final registrasi = await regRepo.getRegistrasiRawById(
          widget.registrationId,
        );
        final root = (registrasi ?? const {});
        // Extract anamnesa from root if present, else treat root as anamnesa
        data = (root['anamnesa'] ?? root) as Map<String, dynamic>;
      }
      if (data != null) {
        // Vital & keluhan
        ...
        _keluhanPasienController.text =
            (kel['keluhan'] ?? data['keluhan'] ?? '').toString();
        _tekananDarahController.text =
            (tv['tekanan_darah'] ?? data['tekanan_darah'] ?? '').toString();
        _frekuensiNadiController.text = ((tv['nadi'] ?? data['nadi']) ?? '')
            .toString();
        _suhuController.text = ((tv['suhu'] ?? data['suhu']) ?? '').toString();
        _frekuensiPernapasanController.text =
            ((tv['pernapasan'] ?? data['pernapasan']) ?? '').toString();

        // Nutrisi
        _beratBadanController.text =
            (nutr['berat_badan'] ?? data['berat_badan'] ?? '').toString();
        _tinggiBadanController.text =
            (nutr['tinggi_badan'] ?? data['tinggi_badan'] ?? '').toString();
        _imtController.text = ((nutr['imt'] ?? data['imt']) ?? '').toString();
        _lingkarKepalaController.text =
            (nutr['lingkar_kepala'] ?? data['lingkar_kepala'] ?? '').toString();

        // Fungsional
        _alatBantuController.text =
            (fungs['alat_bantu'] ?? data['alat_bantu'] ?? '').toString();
        _prothesaController.text = (fungs['prothesa'] ?? data['prothesa'] ?? '')
            .toString();
        _cacatTubuhController.text =
            (fungs['cacat_tubuh'] ?? data['cacat_tubuh'] ?? '').toString();
        _adl = (((fungs['adl'] ?? data['adl'] ?? 0)) == 1)
            ? 'Mandiri'
            : 'Dibantu';
        _resikoJatuh =
            (((fungs['resiko_jatuh'] ?? data['resiko_jatuh'] ?? 0)) == 1)
            ? 'Ada'
            : 'Tidak Ada';
        _riwayatPenyakitDahuluController.text =
            (fungs['riwayat'] ?? data['riwayat'] ?? '').toString();
        _riwayatAlergi =
            (((tv['riwayat_alergi'] ?? data['riwayat_alergi'] ?? 0)) == 1)
            ? 'Ya'
            : 'Tidak';
        ...
        _masalahKeperawatanLainnyaController.text =
            (mk['lainnya'] ?? data['lainnya'] ?? '').toString();

        // Medis
        Map<String, dynamic> pm = (data['pengkajian_medis'] ?? const {});
        _pemeriksaanFisikController.text =
            (pm['pemeriksaan_fisik'] ?? data['pemeriksaan_fisik'] ?? '')
                .toString();
        _diagnosisController.text = (pm['diagnosis'] ?? data['diagnosis'] ?? '')
            .toString();
        _rencanaTerapiController.text =
            (pm['rencana_dan_terapi'] ?? data['rencana_dan_terapi'] ?? '')
                .toString();
        _pemeriksaanPenunjangController.text =
            (pm['pemeriksaan_penunjang'] ?? data['pemeriksaan_penunjang'] ?? '')
                .toString();
        _kontrolController.text = (pm['kontrol'] ?? data['kontrol'] ?? '')
            .toString();
        final jp = (pm['jenis_perawatan'] ?? data['jenis_perawatan'] ?? '')
            .toString();
        if (jp.isNotEmpty) {
        ...
        bool toBoolIntFrom(Map<String, dynamic> m, String key) =>
            (m[key] ?? data[key] ?? 0) == 1;
        _intervensiTimeUpGoOptions.update(
          _intervensiTimeUpGoOptions.keys.elementAt(0),
          (_) => toBoolIntFrom(tug, 'cara_berjalan'),
        );
        _intervensiTimeUpGoOptions.update(
          _intervensiTimeUpGoOptions.keys.elementAt(1),
          (_) => toBoolIntFrom(tug, 'cara_berjalan2'),
        );
        _intervensiTimeUpGoOptions.update(
          _intervensiTimeUpGoOptions.keys.elementAt(2),
          (_) => toBoolIntFrom(tug, 'menopang'),
        );

        final nutrisiBb =
            ((kp['skrining_mst_dewasa'] ?? const {})['nutrisi_bb'] ??
                    data['nutrisi_bb'] ??
                    '')
                .toString();
        _skriningNutrisiMstAnswers['penurunan_berat'] = nutrisiBb == 'Normal'
            ? 'Tidak (0)'
            : 'Tidak yakin (2)';
        final asupMakan =
            ((kp['skrining_mst_dewasa'] ?? const {})['asup_makan'] ??
                    data['asup_makan'] ??
                    '')
                .toString();
        _skriningNutrisiMstAnswers['makan_menurun'] = asupMakan == 'Cukup'
            ? 'Tidak (0)'
            : 'Ya (1)';
        ...
        void setStrongKids(String key, String mapKey) {
          ...
          _skriningNutrisiStrongkidsAnswers[mapKey] = isOne
              ? 'Ya (1)'
              : 'Tidak (0)';
        }
        ...
        _hambatanMobilisasiOptions['lanjut_usia'] =
            (rencana['renc_usia_lanjut'] ?? data['renc_usia_lanjut'] ?? 0) == 1;
        _hambatanMobilisasiOptions['tidak_ada'] =
            (rencana['renc_hmbtn_mobil'] ?? data['renc_hmbtn_mobil'] ?? 0) == 0;
  ...
  Future<void> _saveAnamnesa() async {
    ...
    try {
      ...
      final Map<String, dynamic> payload = {
        ...
        'nadi': _frekuensiNadiController.text.trim(),
        'suhu': _suhuController.text.trim(),
        'pernapasan': _frekuensiPernapasanController.text.trim(),
        'berat_badan': _beratBadanController.text.trim(),
        'tinggi_badan': _tinggiBadanController.text.trim(),
        'imt': _imtController.text.trim(),
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(icon, size: 16, color: kWhite),
          ),
      ...
      children: [
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            ...
            filled: true,
            fillColor: enabled ? kScaffoldBg : kTextGrey.withOpacity(0.1),
            ...
            enabledBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(12),
              borderSide: BorderSide(color: kTextGrey.withOpacity(0.15)),
            ),
