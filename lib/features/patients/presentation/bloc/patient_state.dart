...
class PatientListLoaded extends PatientState {
  ...
  int get registeredCount => patients.where((p) => p.isRegistered ?? false).length;
  int get unregisteredCount => patients.where((p) => !(p.isRegistered ?? false)).length;
}
